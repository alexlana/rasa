...
target "base" {
  dockerfile = "source/docker/Dockerfile.base"
  tags       = ["${IMAGE_NAME}:base-${IMAGE_TAG}"]
  cache-to   = ["type=inline"]
}

target "base-mitie" {
  dockerfile = "source/docker/Dockerfile.base-mitie"
  tags       = ["${IMAGE_NAME}:base-mitie-${IMAGE_TAG}"]
  cache-to   = ["type=inline"]
}

target "base-poetry" {
  dockerfile = "source/docker/Dockerfile.base-poetry"
...
target "base-builder" {
  dockerfile = "source/docker/Dockerfile.base-builder"
...
target "full" {
  dockerfile = "source/docker/Dockerfile.full"
...
target "mitie-en" {
  dockerfile = "source/docker/Dockerfile.pretrained_embeddings_mitie_en"
...
target "spacy-de" {
  dockerfile = "source/docker/Dockerfile.pretrained_embeddings_spacy_de"
...
target "spacy-en" {
  dockerfile = "source/docker/Dockerfile.pretrained_embeddings_spacy_en"
